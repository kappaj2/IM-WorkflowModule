package za.co.ajk.workflow.service.messaging.googlepubsub.impl;
...
import za.co.ajk.workflow.config.PubSubMessagingProperties;
import za.co.ajk.workflow.enums.PubSubMessageType;
import za.co.ajk.workflow.service.messaging.IMMessageProcessor;
import za.co.ajk.workflow.service.messaging.InterModulePubSubMessage;
import za.co.ajk.workflow.service.messaging.googlepubsub.GoogleChannelManager;
...
public class GoogleChannelManagerImpl implements GoogleChannelManager {
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterGenericSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "WorkflowEngineGenericSub");
            adapter
                .setOutputChannel(
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterBillingSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "WorkflowEngineTopicSub");
            adapter
                .setOutputChannel(
                    inputChannel); // looks like the channel to ack on (thus the input channel - confusing!)
            adapter.setAckMode(AckMode.MANUAL);
